...
.. note::
  It's best to take everyone through day 1 as a group, show them the different probes for the oscilloscope, make sure they have AC/DC setting correct. Check that everyone has a nice signal each time before moving on and that basics are clear, like which parts of the breadboard are connected.


1.	Using a multimeter
###################################
    A.	Make sure the multimeter has batteries installed. Put the red lead in the port marked ‘VmAuA’ and the black one in the port marked COM.
    B.	Set the multimeter to ‘Ω’. Use your multimeter to measure the resistance of any 3 different resistors in the kit. Check whether the resistance values you read correspond to the bands on the resistors. How is your multimeter measuring this resistance?
    C.	Now set your multimeter to measure the voltage across a battery. What does it mean if you record a negative voltage?
...
If you need to, refer back to the `reading material <https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all#anatomy-of-a-breadboard>`_ for the overview on how to use a breadboard, it also includes some basic troubleshooting which may prove useful if you run into problems. The most important thing is to remember which holes are connected. Your long breadboard is actually made up of two separate breadboards; + and – rails run through rows 0-30 and 31-60. If you want to use the entire length of the breadboard, you will have to connect these rails together with wires.

In your kit you will find a Teensy 3.2. This is a small microcontroller (a small computer) that we can control in software, and use to generate output or collect input voltage signals. We’re going to attach it to our solderless breadboard.
...
B. Set the slider on the probe itself to ‘1x’. Connect your PicoScope to your computer and open up the Picoscope software.
...
A.  Increase the value of the resistor to 200KOhm. What happens to the current?
B.  Put the resistor back to 1KOhm. Now, increase the capacitance of the capacitor to around 10mF. What happens to the current?
C.  Return the values to 1KOhm and 10uF.
...
A.	Edit the circuit used above to build the equivalent circuit of a polarised, tungsten electrode.
...
B.	Edit the alternating voltage supply to provide 1V at 1Khz, mimicking the signal coming from your cell. 1V is larger than ephys signals, but makes the current flow easier to see. Change the sliders for simulation speed and current speed until you can see where the current is flowing.

C. What happens if you delete |Rm|?

.. note::
  Should see little difference if electrode is only represented by capacitor

D. Can you change this circuit from a polarising, tungsten electrode, to a circuit representing a nonpolarizable electrode?
